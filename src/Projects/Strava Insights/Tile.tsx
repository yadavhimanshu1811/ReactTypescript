const Tile = (props) => {
  ...
  const { name, distance, moving_time, moving_time_raw, distance_raw } = tileData;

  const paceMinPerKm = (distanceMeters: number, timeSeconds: number): string => {
  ...
  return (
    <div className="tile">
      ...
      <div className="tile_item">{paceMinPerKm(distance_raw, moving_time_raw)}</div>
    </div>
  );
};
